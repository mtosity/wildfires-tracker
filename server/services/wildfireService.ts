...
function convertFirmsToWildfire(
  ...
  Object.entries(fireGroups).forEach(([gridKey, fires]) => {
    ...
    const avgConfidence =
      fires.reduce((sum, fire) => sum + fire.confidence, 0) / fires.length;
    ...
    wildfires.push({
      ...
      startDate: existingFire
        ? existingFire.startDate
...
export async function fetchAndUpdateWildfires() {
  try {
    ...
    // Using VIIRS instead of MODIS as it typically captures more active fires
    const source = "VIIRS_NOAA20_NRT";
    const area = "world";
    const dayRange = 7; // Increase to 7 days of data to capture more fires
