...
const Map: React.FC<MapProps> = ({
  ...
  const points = useMemo((): PointFeature[] => {
    return wildfires.filter(isActiveWildfire).map((wildfire) => ({
      type: "Feature",
      properties: {
        id: wildfire.id,
        wildfire,
  ...
  const clusters = useMemo(() => {
    if (!viewportBounds || !mapLoaded || !supercluster || points.length === 0)
      return [];
  ...
  useEffect(() => {
    ...
    if (mapContainer.current && !map.current) {
      ...
      map.current = new mapboxgl.Map({
        ...
        center: [
          initialPosition?.longitude || defaultPosition.longitude,
  ...
  const createUserLocationMarker = useCallback((lng: number, lat: number) => {
    ...
    pulseRing.className =
      "absolute w-full h-full bg-blue-500 opacity-70 rounded-full animate-ping";
    el.appendChild(pulseRing);

    // Add the center marker dot
    const centerDot = document.createElement("div");
    centerDot.className =
      "absolute w-3 h-3 bg-blue-600 rounded-full border-2 border-white z-10";
  ...
  useEffect(() => {
    ...
    clusters.forEach((cluster) => {
      ...
      if (cluster.properties.cluster) {
        ...
        const size = Math.min(
          55,
          Math.max(35, 35 + Math.log10(pointCount) * 10)
        );
        ...
        innerCircle.className =
          "bg-primary/90 text-white rounded-full flex items-center justify-center font-bold shadow-md w-full h-full";
        innerCircle.style.fontSize = `${Math.max(
          12,
          Math.min(18, 12 + Math.log10(pointCount) * 3)
        )}px`;
        ...
        if (
          ...
          dotEl.style.backgroundColor =
            wildfire.severity === "high"
              ? "#FF8A80"
              : wildfire.severity === "medium"
              ? "#FFD180"
              : wildfire.severity === "low"
              ? "#CCFF90"
              : "#B9F6CA";
  ...
  const addPerimeterForWildfire = useCallback(
    (wildfire: Wildfire) => {
      ...
      try {
        ...
        if (!map.current.getSource(sourceId)) {
          try {
            ...
            console.error(
              "Error adding source for wildfire:",
              wildfire.id,
              sourceError
            );
            return;
        ...
        try {
          ...
          console.error(
            "Error adding layer for wildfire:",
            wildfire.id,
            layerError
          );
        }
        ...
        console.error(
          "Error rendering perimeter for wildfire:",
          wildfire.id,
          error
        );
      }
